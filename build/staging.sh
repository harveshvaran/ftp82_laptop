#!/bin/bash -ex
#deploy
curl --fail -T ftpxx.war "http://manager:manager@$STAGING_HOST:8080/manager/text/deploy?path=/ftpxx&update=true"
...
while true
do
  response=$(curl --write-out %{http_code} --silent --output /dev/null http://$STAGING_HOST:8080/ftpxx/favicon.ico)
